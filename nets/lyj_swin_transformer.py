...
class B_SILU(nn.Module):
    def __init__(self):
        super(B_SILU, self).__init__()
    ...
    def forward(self, x):
        ...
        b_silu = self.beta * silu + (1 - self.beta) * x * torch.sigmoid(0.5 * x)
        return b_silu
...
# # -------------------------------------------------------#
class Mlp(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=B_SILU, drop=0.):
        super().__init__()
...
class SSPP(nn.Module):
    ...
    def __init__(self, in_channels, levels=[1, 2, 4]):
        ...
        self.conv = nn.Conv2d(in_channels * len(levels), in_channels, kernel_size=1)

    def forward(self, x):
        features = [pool(x) for pool in self.pools]
        features = [F.interpolate(f, size=x.shape[2:], mode='bilinear', align_corners=False)
                    for f in features]
        combined = torch.cat(features, dim=1)
        return self.conv(combined)


class LYJSwinTransformer(nn.Module):
    def __init__(self, img_size=[224, 224], patch_size=4, in_chans=3, num_classes=100,
                 embed_dim=96,                      
                 depths=[2, 2, 2, 2],               # 保持不变
                 num_heads=[3, 6, 12, 24],           # 保持不变 
        ...
        # 添加SSPP模块
        self.sspp = SSPP(in_channels=self.num_features, levels=[1, 2, 4])
...
if __name__ == '__main__':
    x = torch.ones((24, 3, 224, 224))  # 统一到224x224
    model = LYJSwinTransformer(img_size=[224, 224], num_classes=100)
    model(x)
